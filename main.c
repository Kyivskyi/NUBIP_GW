...
void viewHousing() {
    ...
    for (int i = 0; i < housingCount; i++) {
        if (strcmp(housings[i].name, housingName) == 0) {
            ...
            printf("Рейтинг: %.2f (кількість оцінок: %d)\n", housings[i].rating, housings[i].ratingCount);
            return;
        }
    }
    ...
    for (int i = 0; i < housingCount; i++) {
        if (strcmp(housings[i].region, region) == 0) {
            printf("Назва: %s, Ціна за добу: %.2f, Доступність: %s\n", 
                   housings[i].name, housings[i].price, 
                   housings[i].isBooked ? "Не доступно" : "Доступно");
        }
    }
}
...
void rateHousing() {
    ...
    printf("Введіть назву житла для оцінки: ");
    scanf(" %[^\n]s", housingName);

    for (int i = 0; i < housingCount; i++) {
        if (strcmp(housings[i].name, housingName) == 0) {
            printf("Введіть оцінку від 1 до 5: ");
            scanf("%f", &rating);

            if (rating >= 1 && rating <= 5) {
                housings[i].rating = (housings[i].rating * housings[i].ratingCount + rating) / (housings[i].ratingCount + 1);
                housings[i].ratingCount++;
                printf("Дякуємо за вашу оцінку! Нова середня оцінка житла: %.2f\n", housings[i].rating);
            } else {
                printf("Неправильна оцінка. Будь ласка, введіть число від 1 до 5.\n");
            }
            return;
        }
    }
